    ...
    <div className="fixed inset-0 z-50 flex items-end bg-black/70">
      <div className="bg-[#161616] rounded-t-2xl w-full max-w-[430px] mx-auto p-6 border-t border-[#222]">
        ...
        <div className="bg-[#00C85510] border border-[#00C85330] rounded-xl p-3 mb-4">
          <p className="text-[#00C853] text-xs font-semibold mb-1">
            How It Works
          </p>
          <p className="text-[#888] text-[11px]">
            Profit moves into your active deposit, increasing your daily ROI
            base. Your principal and total balance remain unchanged.
          </p>
        </div>
        <div className="card mb-4">
          ...
          <div className="flex gap-2">
            {['25%', '50%', '75%', '100%'].map((p) => {
              const val = (
                (Number(user.total_profit) * parseInt(p)) /
                100
              ).toFixed(2);
              return (
                <button
        ...
        {preview && (
          <div className="card mb-4 space-y-0">
            {[
              {
                label: 'Active Deposit After',
                value: fmt.usd(preview.new_active_deposit),
                color: 'text-white',
              },
              {
                label: 'Est. Daily ROI Increase',
                value: `+${fmt.usd(preview.daily_roi_increase)}/day`,
                color: 'text-[#00C853]',
              },
              {
                label: 'Balance (unchanged)',
                value: fmt.usd(user.total_balance),
                color: 'text-[#888]',
              },
            ].map((r, i) => (
              <div
      ...
      <div className="bg-[#161616] rounded-t-2xl w-full max-w-[430px] mx-auto p-6 border-t border-[#222]">
        ...
        <div className="space-y-0">
          {[
            ...
            {
              label: 'Active Deposit (ROI Base)',
              value: fmt.usd(user.active_deposit),
            },
            {
              label: 'Total Profit',
              value: fmt.usd(user.total_profit),
              color: 'text-[#00C853]',
            },
            {
              label: 'Total Balance',
            ...
            {
              label: 'Cycle Target (200%)',
              value: fmt.usd(Number(user.principal) * 2),
              color: 'text-[#F9A825]',
            },
            ...
            { label: 'Completed Cycles', value: user.completed_cycles || 0 },
            {
              ...
              value: user.next_withdrawal_date
                ? fmt.date(user.next_withdrawal_date)
                : 'Available now',
            },
          ].map((row, i) => (
            <div
              key={i}
              className="flex justify-between py-2.5 border-b border-[#1a1a1a] last:border-0"
            >
              <span className="text-[#666] text-sm">{row.label}</span>
              <span
                className={`text-sm font-semibold ${(row as any).color || 'text-white'}`}
              >
                {row.value}
              </span>
            </div>
    ...
    <div className="screen">
          ...
          <button
            onClick={() => {
              if (!canReinvest) {
                toast.error(
                  !hasProfit
                    ? 'No profit available to reinvest'
                    : 'Account state does not allow reinvestment',
                );
                return;
              }
      ...
      <div className="mx-4 mt-3 card">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[#00C853] text-sm font-bold">
            {fmt.pct(cyclePercent / 2)}
          </span>
        </div>
        <ProgressBar percent={cyclePercent / 2} />
        <div className="flex justify-between mt-2">
          <span className="text-[11px] text-[#555]">
            {fmt.usd(data.total_balance)}
          </span>
          <span className="text-[11px] text-[#555]">
            Target {fmt.usd(Number(data.principal) * 2)}
          </span>
        </div>
      </div>
      ...
      {(isActive || isGrace) && (
        ...
        >
          <TrendingUp
            size={16}
            className={isGrace ? 'text-[#F9A825]' : 'text-[#00C853]'}
          />
          <p
            className={`text-[13px] font-medium ${isGrace ? 'text-[#F9A825]' : 'text-[#00C853]'}`}
          >
            {isGrace
          ...
          {[
            {
              ...
              color: '#00C853',
              bg: '#00C85320',
              hoverBorder: 'hover:border-[#00C853]',
            },
            {
              href: '/withdraw',
              label: 'Withdraw',
              sub: 'Withdraw your profit',
              Icon: ArrowUpFromLine,
              color: '#F9A825',
              bg: '#F9A82520',
              hoverBorder: 'hover:border-[#F9A825]',
            },
            {
              href: '/earn',
              label: 'Referral',
              sub: 'Invite & earn more',
              Icon: Users,
              color: '#1565C0',
              bg: '#1565C020',
            ...
            >
              ...
              <span className="text-[10px] text-[#555] text-center leading-tight">
                {sub}
              </span>
            </Link>
          ))}
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            },
            {
              ...
              value: data.referral_wallet_balance,
              color: 'text-[#1565C0]',
            },
            {
              icon: '🎁',
              label: 'Promotion Earnings',
              value: data.promotion_wallet_balance,
              color: 'text-[#7B1FA2]',
            },
          ].map((item, i) => (
      ...
      <div className="mx-4 mt-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/earn" className="text-[#00C853] text-xs font-semibold">
            View Progress
          </Link>
        </div>
        <div className="card">
          <div className="grid grid-cols-3 gap-0 mb-3">
            <div className="text-center pr-3 border-r border-[#1a1a1a]">
              ...
              <div className="text-[#888] text-[10px] mb-0.5">
                Loyalty Score
              </div>
              <div className="text-[#00C853] font-bold text-sm">
                {fmt.pct(data.loyalty_score)}
              </div>
            ...
            <div className="text-center pl-3">
              ...
              <div className="text-[#F9A825] font-bold text-sm">
                {data.rank_level >= 6
                  ? '100%'
                  : `${Math.round(((data.completed_cycles || 0) / 3) * 100)}%`}
              </div>
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-[#666] text-xs">
              Choose a plan and make your first deposit to start earning daily
              ROI.
            </p>
          </div>
      ...
      <div
        className="mx-4 mt-3 mb-6 bg-[#111] border border-[#1a1a1a] rounded-xl p-3
                      flex items-center gap-3"
      >
        <span className="text-xl shrink-0">💡</span>
        <p className="text-[#555] text-xs leading-relaxed">
          Make your first deposit today and let Lianka work for you. Consistent
          investment leads to consistent growth.
        </p>
      </div>
