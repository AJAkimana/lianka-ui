...
function WithdrawalCancelCountdown({
  requestedAt,
  onCancel,
}: {
  requestedAt: string;
  ...
  const [remaining, setRemaining] = useState(
    Math.max(0, CANCEL_WINDOW - elapsed),
  );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <Clock
          size={14}
          className={canCancel ? 'text-[#F9A825]' : 'text-[#555]'}
        />
        <div>
          <p
            className={`text-xs font-semibold ${canCancel ? 'text-white' : 'text-[#555]'}`}
          >
            {canCancel
...
export default function WithdrawPage() {
  ...
  const getGateStatus = () => {
    ...
    if (['TERMINATED', 'FROZEN'].includes(user.account_state)) {
      return {
        blocked: true,
        message: 'Account closed — withdrawals not available',
        color: 'red',
      };
    }
    if (user.account_state === 'INACTIVE') {
      return {
        blocked: true,
        message: 'No active cycle — deposit to restart',
        color: 'gray',
      };
    }
    if (user.kyc_status !== 'VERIFIED') {
      ...
      return {
        blocked: true,
        message: msgs[user.kyc_status] || 'Complete KYC',
        color: 'yellow',
        link: '/profile',
      };
    }
    ...
    if (user.next_withdrawal_date) {
      ...
      if (today < user.next_withdrawal_date) {
        return {
          blocked: true,
          message: `Next withdrawal available on ${fmt.date(user.next_withdrawal_date)}`,
          color: 'gray',
        };
      }
    }
  ...
  const walletBalance =
    {
      profit: Number(user?.total_profit || 0),
  ...
  const maxWithdrawal = () => {
    ...
    return (
      Number(user?.total_profit || 0) * (percents[user?.timeframe] || 0.25)
    );
  };
  ...
  return (
    <div className="screen">
      ...
      {user?.kyc_status === 'VERIFIED' ? (
        <div
          className="mx-4 mt-3 bg-[#00C85310] border border-[#00C85330] rounded-xl p-3
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-[#666] text-[11px]">
              Your identity is verified. You can withdraw your profits.
            </p>
          </div>
        ...
        <div
          className="mx-4 mt-3 bg-[#F9A82510] border border-[#F9A82530] rounded-xl p-3
                        flex items-center gap-3"
        >
      ...
      <p className="text-[#555] text-[11px] mx-4 mt-2 px-1">
        Withdrawals are allowed only from profit. Your principal (active
          ...
          {[
            {
              id: 'profit',
              label: 'Profit Wallet',
              balance: user?.total_profit,
              color: '#00C853',
            },
            {
              id: 'referral',
              label: 'Referral Wallet',
              balance: user?.referral_wallet_balance,
              color: '#1565C0',
            },
            {
              id: 'promotion',
              label: 'Promotion Wallet',
              balance: user?.promotion_wallet_balance,
              color: '#7B1FA2',
            },
          ].map((w) => (
            <button
            ...
            >
              ...
              <p style={{ color: w.color }} className="font-bold text-sm mt-1">
                {fmt.usd(w.balance)}
              </p>
            </button>
          ))}
      ...
      <div className="mx-4 mt-4">
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-3">
            ...
            />
            <button
              onClick={() => setAmount(maxWithdrawal().toFixed(2))}
          ...
          <div className="space-y-1.5 text-xs">
            <div className="flex justify-between">
              ...
              <span className="text-[#00C853] font-semibold">
                {fmt.usd(walletBalance)}
              </span>
            </div>
            {walletTab === 'profit' && (
              <div className="flex justify-between">
                <span className="text-[#555]">
                  Maximum allowed ({user?.timeframe} plan)
                </span>
                <span className="text-[#F9A825] font-semibold">
                  {fmt.usd(maxWithdrawal())}
                </span>
              </div>
            )}
            <div className="flex justify-between">
            ...
            {amount && Number(amount) > 0 && (
              <div className="flex justify-between pt-2 border-t border-[#1a1a1a]">
                ...
                <span className="text-[#00C853] font-bold">
                  {fmt.usd(Math.max(0, Number(amount) - 2.1))}
                </span>
              </div>
            )}
          </div>
            ...
            <div className="mt-3 pt-3 border-t border-[#1a1a1a]">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-[#F9A825] text-[11px] font-semibold">
                      Next Withdrawal Date
                    </p>
                    <p className="text-[#666] text-[11px]">
                      {fmt.date(user.next_withdrawal_date)} 00:00 (Platform
                      Time)
                    </p>
                    <p className="text-[#555] text-[10px]">
                      You can withdraw again after this date.
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-[#555] text-[10px]">Timeframe Plan</p>
                  <p className="text-[#1565C0] text-[11px] font-semibold">
                    {user?.timeframe} Plan
                  </p>
      ...
      <div className="mx-4 mt-4">
        ...
        <div className="card">
          ...
          {!withdrawAddress && (
            <p className="text-[#C1121F] text-xs mt-2">
            ...
            {[
              {
                ...
                ok: ['ACTIVE', 'GRACE'].includes(user?.account_state),
              },
              {
                label: 'KYC Verification',
                value: user?.kyc_status,
                ok: user?.kyc_status === 'VERIFIED',
              },
              {
                label: 'Withdrawal Address',
                value: withdrawAddress ? 'Set & Verified' : 'Not Set',
                ok: !!withdrawAddress,
              },
              {
                label: 'Sufficient Profit Balance',
                value: Number(user?.total_profit) > 0 ? 'Yes' : 'No',
                ok: Number(user?.total_profit) > 0,
              },
              {
                label: 'No Pending Request',
                value: pending ? 'Pending' : 'Yes',
                ok: !pending,
              },
              {
                label: 'Minimum Amount Met',
                value: Number(amount) >= 10 ? 'Yes' : 'No',
                ok: Number(amount) >= 10,
              },
            ].map((c, i) => (
              <div
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-xs font-semibold ${c.ok ? 'text-[#00C853]' : 'text-[#F9A825]'}`}
                  >
                    {c.value}
                  </span>
                  {c.ok ? (
          ...
          <div>
            ...
            <div className="bg-[#F9A82510] border border-[#F9A82540] rounded-xl p-4 mb-3">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-[#666] text-xs mt-1">
                    You will receive a notification once the transaction is
                    completed.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-[#0a0a0a] rounded-xl p-3 mb-3 space-y-0">
                {[
                  ...
                  { label: 'Withdrawal Amount', val: fmt.usd(pending.amount) },
                  {
                    ...
                    val: fmt.usd(pending.final_amount),
                    color: 'text-[#00C853]',
                  },
                  { label: 'Network', val: `USDT (${pending.network})` },
                  {
                    label: 'Request Date & Time',
                    val: new Date(pending.requested_at).toLocaleString(),
                  },
                  {
                    label: 'Request ID',
                    val: `WD${new Date(pending.requested_at).toISOString().slice(0, 10).replace(/-/g, '')}${pending.id?.slice(-6).toUpperCase()}`,
                  },
                ].map((row, i) => (
                  <div
                    key={i}
                    className="flex justify-between py-2 border-b border-[#1a1a1a] last:border-0 text-xs"
                  >
                    <span className="text-[#555]">{row.label}</span>
                    <span
                      className={`font-semibold ${row.color || 'text-white'}`}
                    >
                      {row.val}
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-0 mb-3">
                {[
                  {
                    label: 'Request\nReceived',
                    sub: new Date(pending.requested_at).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit',
                    }),
                    active: true,
                  ...
                  <div key={i} className="flex items-center flex-1">
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-3 h-3 rounded-full ${s.active ? 'bg-[#1565C0]' : 'bg-[#333]'}`}
                      />
                      <p
                        className={`text-[8px] text-center mt-1 whitespace-pre-line leading-tight
                        ${s.active ? 'text-[#1565C0]' : 'text-[#555]'}`}
                      >
                        {s.label}
                      </p>
                    ...
                    {i < 3 && (
                      <div className="flex-1 h-px bg-[#1a1a1a] mx-0.5 mb-5" />
                    )}
                  </div>
                ))}
              </div>
              <p className="text-[#555] text-[11px] flex items-center gap-1">
                <Clock size={11} /> Average processing time: 24–72 hours. You
                will be notified via in-app notification and email once the
            ...
            <div className="bg-[#F9A82510] border border-[#F9A82530] rounded-xl p-3 mb-3">
              <div className="flex items-start gap-3">
                <div className="flex-1">
                  <p className="text-[#F9A825] text-xs font-bold mb-1">
                    ⚠ Important
                  </p>
                  <ul className="space-y-1">
                    {[
                      ...
                      <li key={i} className="text-[#888] text-[11px]">
                        • {item}
                      </li>
                    ))}
                  </ul>
            ...
            <div className="bg-[#7B1FA210] border border-[#7B1FA230] rounded-xl p-3 mt-3">
              ...
              <p className="text-[#666] text-xs mt-0.5">
                You cannot create another withdrawal request while this one is
                pending.
              </p>
            </div>
          </div>
          ...
          <div>
            <button
              onClick={() => {
                ...
                if (!withdrawAddress) {
                  toast.error('Set a withdrawal address first');
                  return;
                }
                setShowConfirm(true);
              }}
            ...
            {gate?.blocked && (
              <p className="text-[#856404] text-xs text-center mt-2">
                {gate.message}
              </p>
            )}
          </div>
          ...
          <div className="bg-[#161616] rounded-t-2xl w-full max-w-[430px] mx-auto p-6 border-t border-[#222]">
            ...
            <div className="space-y-0 mb-4">
              ...
              <InfoRow
                ...
                value={fmt.usd(Number(amount) - 2.1)}
                valueClass="text-[#00C853]"
              />
              <InfoRow label="Network" value={`USDT (${withdrawNetwork})`} />
              <InfoRow
                label="Address"
                value={`${withdrawAddress.slice(0, 10)}...${withdrawAddress.slice(-8)}`}
                valueClass="text-[#888] font-mono text-[11px]"
              />
            </div>
            <div className="bg-[#F9A82510] border border-[#F9A82530] rounded-lg p-3 mb-4">
              <p className="text-[#F9A825] text-xs">
                ⚠ Withdrawing may reduce your balance below principal and result
